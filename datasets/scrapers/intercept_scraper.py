...
def get_paragraph_texts(soup):
	...
	paragraph_texts = [c.text for c in paragraph_items \
		...
		or str(c).startswith('<p style="text-align: left">')
		...
		or str(c).startswith('<p class="Bookshelf-Book-Title-Info">')
		or str(c).startswith('<p class="Bookshelf-Standard-Paragraph">')
		or str(c).startswith('<p style="color: #')
		or str(c).startswith('<p class="s1">')
		or str(c).startswith('<p class="p4">')
		or str(c).startswith('<p class="story')
		or str(c).startswith('<p class="loose">')
		or str(c).startswith('<p id="U8')
		# or str(c).startswith('<div data-reactid="203">')
	]
...
def is_english(soup):
	...
	lang_index = main_item.find("html lang")

	return 'lang="en"' in main_item[lang_index:lang_index+100]
