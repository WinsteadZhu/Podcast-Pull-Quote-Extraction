...
if quick_mode:
	print(f"QUICK MODE {parsing.quick}")

articles_data = preprocess_pull_quotes(directories=settings.PQ_SAMPLES_DIRS)
if quick_mode:
	articles_data = articles_data[:parsing.quick]
...
model.mode = "C_deep"
for width, nb_experts in itertools.product([32, 64], [4]):
	for trial in range(trials):
		model.train_model(nb_experts=nb_experts, width=width)
		model_name = f"{model.name}_{model.mode}_{parsing.quick}_{width}_{nb_experts}_{trial+1}.keras"
		model.save_model(settings.MODEL_SAVE_DIR + model_name)
		val_accuracy = E.evaluate(model=model, articles=val_articles, verbose=0)
		test_accuracy = E.evaluate(model=model, articles=test_articles, verbose=0)
		nb_params = model.model.count_params()
		res_str = "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{:.1f}\t{:.1f}".format(model_name, trial, model.name, model.mode, width, nb_experts, nb_params, 100*val_accuracy, 100*test_accuracy)
		print(res_str)
		results_file.write(res_str+"\n")
		results_file.flush()
